<style>
    /* ───── جدول ───── */
    #brokerTable thead th ,#brokerTable tfoot td{
        background: #f8f9fa;
        font-size: 1rem;
        /* أكبر قليلاً */
        font-weight: 700;
    ...
    <script>
        $(function() {
            ...
            $('#brokerPrint').on('click', function () {
                ...
                brokers.forEach(acc => {
                    $('<input>', {type:'hidden', name:'broker[]', value:acc}).appendTo(form);
                });
                        ...
                        res.rows.forEach((row, i) => {
                            tbody.append(`
                                <tr>
                                    ...
                                    <td class="fw-bold">${row.allocations}</td>
                                    <td class="fw-bold">${row.implementations}</td>
                                    <td class="fw-bold">${row.active_allocations}</td>
                                </tr>
                            `);
                        });
                    },
