<x-front-layout>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centerd" role="document">
            <div class="modal-content">
                <form id="editForm">
                    @include('dashboard.offices.editModal')
                </form>
            </div>
        </div>
    </div>
    @push('scripts')
        ...
        <script>
            ...
            const urlIndex = `{{ route('dashboard.offices.index') }}`;
            const urlFilters = `{{ route('dashboard.offices.filters', ':column') }}`;
            const urlCreate = `{{ route('dashboard.offices.create') }}`;
            const urlStore = `{{ route('dashboard.offices.store') }}`;
            const urlEdit = `{{ route('dashboard.offices.edit', ':id') }}`;
            const urlUpdate = `{{ route('dashboard.offices.update', ':id') }}`;
            const urlDelete = `{{ route('dashboard.offices.destroy', ':id') }}`;

            // ability
            const abilityCreate = "{{ Auth::user()->can('create', 'App\\Models\\Office') }}";
            const abilityEdit = "{{ Auth::user()->can('update', 'App\\Models\\Office') }}";
            const abilityDelete = "{{ Auth::user()->can('delete', 'App\\Models\\Office') }}";
            ...
            const columnNamesCopy = [ 'name', 'location', 'is_active', 'notes' ];
        </script>
