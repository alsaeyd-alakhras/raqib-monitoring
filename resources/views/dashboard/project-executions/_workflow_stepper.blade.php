...
@if ($isRejected)
    <div class="alert alert-danger py-2 mb-3">
        ...
        — المسار خارج مسار الاعتماد المعتاد.
    </div>
@endif
    ...
    <div class="project-workflow-stepper-track">
        @foreach ($workflowSteps as $index => $stepKey)
            @php
                ...
                if ($timestamp && ($stepClass === 'is-done' || ($stepKey === 'passage_complete' && $execution->workflow_status === 'passage_complete'))) {
            ...
            <div class="project-workflow-step {{ $stepClass }}">
                ...
                @if ($displayAt)
                    <span class="project-workflow-step-date">{{ $displayAt->format('Y-m-d H:i') }}</span>
                @endif
            </div>
        @endforeach
    </div>
